...
@SpringBootTest
public class Webspider_sentence_complete {
    ...
    public void test() throws InterruptedException, IOException {
        ...
        for(int j=startPage;j<totalPages;j++) {
            ...
            for (int i = 0; i < words.size(); i++) {
                Word w=words.get(i);
                Thread.sleep(100);
                if(!sentence1Serv.isExist(w.getWord()) && frequencyServ.getFrequency(w.getWord())<=3)
                {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            try{
                if(i==0){
                    ...
                    System.out.println("save:"+sentence1.getWord()+" :"+sentence1.getSentence_english());
                }
                else {
                    Sentence2 sentence2 = new Sentence2(word, eng_sentence_words,pronunBytes, chi_sentence_words);
                    sentence2Serv.save(sentence2);
                    System.out.println("save:"+sentence2.getWord()+" :"+sentence2.getSentence_english());
                }
            }
            catch (Exception e)
            {
